...
public class AudioPlayer implements MethodCallHandler, Player.Listener, MetadataOutput {
    ...
    public void onPlaybackStateChanged(int playbackState) {
        ...
        case Player.STATE_BUFFERING:
            ...
            // If we're seeking, don't change to buffering state to avoid UI flicker
            if (processingState != ProcessingState.buffering && 
                ...
                // We're in a seek operation - if we've been buffering too long, 
                // just complete the seek anyway to prevent UI hanging
                handler.postDelayed(() -> {
                    if (seekResult != null) {
                        completeSeek();
                    }
                }, 500); // 500ms timeout for seek operations
            }
            startWatchingBuffer();
            break;
    ...
    private void load(final MediaSource mediaSource, final long initialPosition, final Integer initialIndex, final Result result) {
        ...
        // For buffering the entire media, we need to recreate the player with the new load control
        if (player != null) {
            player.release();
        }
        
        // Create a load control that buffers the entire audio file
        DefaultLoadControl.Builder loadControlBuilder = new DefaultLoadControl.Builder()
            .setBufferDurationsMs(
                DefaultLoadControl.DEFAULT_MIN_BUFFER_MS,
                // Use a very large max buffer size to load the entire audio
                Integer.MAX_VALUE,
                DefaultLoadControl.DEFAULT_BUFFER_FOR_PLAYBACK_MS,
                DefaultLoadControl.DEFAULT_BUFFER_FOR_PLAYBACK_AFTER_REBUFFER_MS
            )
            // Enable a very large back buffer with retention to prevent rebuffering on backward seeks
            // This tells ExoPlayer to retain the already played content in memory
            .setBackBuffer(Integer.MAX_VALUE, true);
        
        // Recreate the player with the new load control
        ...
        // Configure the player with the previously saved playback state
    ...
    public void seek(final long position, final Integer index, final Result result) {
        ...
        try {
            ...
            // Check if we're seeking backward
            boolean isBackwardSeek = position < player.getCurrentPosition();
            
            // Since we're buffering the entire file at start, seeking shouldn't require rebuffering
            player.seekTo(windowIndex, position);
            
            // If we're already in ready state or we're seeking backward to already buffered content,
            // complete the seek immediately
            if (processingState == ProcessingState.ready || isBackwardSeek) {
                // Force immediate completion of the seek without waiting for buffering
                handler.post(() -> {
                    if (seekResult != null) {
                        completeSeek();
                    }
