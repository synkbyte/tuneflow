...
class MyAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
  ...
  void broadcastState([PlaybackEvent? event]) {
    event = event ?? audioPlayer.playbackEvent;
    bool isFav = false;
    if (navigatorKey.currentContext != null) {
      SongModel? nowPlaying =
          navigatorKey.currentContext!.read<PlayerProvider>().nowPlaying;
      if (nowPlaying != null) {
        isFav = navigatorKey.currentContext!
            .read<FavoriteProvider>()
            .isInLikedSong(nowPlaying);
      }
    }
    ...
    playbackState.add(
      playbackState.value.copyWith(
        controls: [
          ...
          MediaControl.custom(
            androidIcon: isFav ? 'drawable/fav_filled' : 'drawable/fav_outline',
            label: isFav ? 'Favorite' : 'Unfavorite',
            name: 'favorite',
          ),
          MediaControl.custom(
  ...
  Future<void> customAction(String name, [Map<String, dynamic>? extras]) async {
    if (name == 'favorite') {
      Fluttertoast.cancel();
      if (navigatorKey.currentContext!.read<FavoriteProvider>().isInLikedSong(
        playerProvider.nowPlaying!,
      )) {
        navigatorKey.currentContext!
            .read<FavoriteProvider>()
            .removeFavoriteSong(playerProvider.nowPlaying!);
        Fluttertoast.showToast(msg: 'Removed from Favorite');
      } else {
        navigatorKey.currentContext!.read<FavoriteProvider>().addFavoriteSong(
          playerProvider.nowPlaying!,
        );
