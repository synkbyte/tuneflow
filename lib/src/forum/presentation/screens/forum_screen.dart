...
import 'package:new_tuneflow/src/forum/presentation/screens/my_posts.dart';
...
class _ForumScreenState extends State<ForumScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        controller: provider.exploreController,
        children: [
          const Gap(20),
          ListView.separated(
            itemCount: provider.exploreFeed.length + 1,
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
            padding: const EdgeInsets.symmetric(horizontal: 20),
            itemBuilder: (context, index) {
              if (index < provider.exploreFeed.length) {
                ForumModel model = provider.exploreFeed[index];
                return _buildPostCard(model);
              } else {
                if (provider.currentPageExplore < provider.totalPageExplore) {
